...
class Parser {
    ...
    switch (client) {
      ...
      case 'YTMUSIC':
        processed_data = ({
          ...
          MUSICFEED: () => this.#processMusicFeed()
        })[data_type]();
        break;
      default:
  ...
  #processMusicFeed() {
    const first_node = Utils.findNode(this.data, 'contents', 'musicImmersiveCarouselShelfRenderer', 10, false)
    const second_node = Utils.findNode(this.data, 'contents', 'musicCarouselShelfRenderer', 10, false)
    const contents = first_node.musicImmersiveCarouselShelfRenderer.contents.concat(second_node.musicCarouselShelfRenderer.contents)

    const parseItems = (contents) => {
      return YTMusicDataItems.MusicFeedItem.parse(contents)
    }

    console.log(parseItems(contents))
