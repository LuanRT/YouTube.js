...
class ContinuationItem {
  ...
  /**
   * Resolve the continuation and then add it to the object itself, so that the appended content may be traversed by the Simplify API.
   * @returns 
   */
  async call(actions, client) {
    if (this.is_resolved || this.is_rejected) return;
    this.pending_promise = this.endpoint.call(actions, client);
