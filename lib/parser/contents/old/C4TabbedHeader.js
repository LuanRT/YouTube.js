...
class C4TabbedHeader {
  ...
  constructor(item) {
    ...
    this.sponsor_button = item.sponsorButton && Parser.parseItem(item.sponsorButton);
    this.subscribe_button = item.subscribeButton && Parser.parseItem(item.subscribeButton);
    this.header_links = item.headerLinks && Parser.parseItem(item.headerLinks);
  }
}
