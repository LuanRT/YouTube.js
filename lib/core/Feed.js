...
const SimplePlaylist = require('./PlaylistItem');
const SimpleVideo = require('./VideoItem');
...
class Feed {
     ...
     */
    #session;
    constructor(session, data, already_parsed = false) {
    ...
    getVideos() {
        return Simplify.matching({
            type: Simplify.matching(SimpleVideo.set),
        }).runOn(this.#page);
    }
     ...
     */
    getPlaylists() {
        return this.#page.fuzzyFind({
            type: Simplify.matching(SimplePlaylist.set),
        }).runOn(this.#page);
    }
    ...
    getChannels() {
        return Simplify.matching({
            type: Simplify.matching(new Set('Channel', 'GridChannel')),
        }).runOn(this.#page);
    }
    ...
    get has_continuation() {
        // TODO: could probably be more efficient with 
        return Simplify.matching({
