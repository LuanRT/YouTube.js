...
async function browse(session, action, args = {}) {
  if (!session.logged_in && ![ 'home_feed', 'music_feed', 'lyrics',
  ...
  switch (action) {
    ...
    case 'home_feed':
      ...
      break;
    case 'music_feed':
      const musicfeed_context = JSON.parse(JSON.stringify(session.context)); // deep copy the context obj so we don't accidentally change it

      musicfeed_context.client.originalUrl = Constants.URLS.YT_MUSIC;
      musicfeed_context.client.clientVersion = Constants.YTMUSIC_VERSION;
      musicfeed_context.client.clientName = 'WEB_REMIX';
      
      data.context = musicfeed_context;
