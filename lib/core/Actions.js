...
class Actions {
  ...
  async browse(id, args = {}) {
    ...
    if (args.is_ytm || args.is_yta) {
      const context = JSON.parse(JSON.stringify(this.#session.context)); 
      
      context.client.originalUrl = args.is_ytm && Constants.URLS.YT_MUSIC || Constants.URLS.YT_BASE;
      context.client.clientVersion = args.is_ytm && Constants.CLIENTS.YTMUSIC.VERSION || Constants.CLIENTS.ANDROID.VERSION;
      context.client.clientName = args.is_ytm && Constants.CLIENTS.YTMUSIC.NAME || Constants.CLIENTS.ANDROID.NAME;
