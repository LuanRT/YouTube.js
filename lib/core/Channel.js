...
class Channel {
    ...
    getTab(name) {
        return Simplify.matching({
            type: 'Tab',
            title: Simplify.matching(new RegExp('^' + name + '$', 'i')), // XXX: this is kept as regex since we want case insensitive matching. Could be optimized.
        }).runOn(this.#page)[0];
    }
    ...
    async getFeedFromTab(tab) {
        ...
        if (target_tab.selected)
            return new Feed(this.#session, this.page, true);
