...
class Innertube {
  ...
  /**
   * Retrieves YouTube Music's home feed.
   * @returns {Promise.<{ content: [{ id: string; title: string; description: string; subtitle: string; thumbnail: [object] }] }>}
   */
   async getMusicFeed() {
    const response = await Actions.browse(this, 'music_feed');
    if (!response.success) throw new Utils.InnertubeError('Could not retrieve music feed', response);

    const musicFeed = new Parser(this, response, {
      client: 'YTMUSIC',
      data_type: 'MUSICFEED'
    }).parse();
