...
class Innertube {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (auth_info.access_token) {
        if (!oauth.isTokenValid()) {
          const new_tokens = await oauth.refreshAccessToken();
          auth_info.refresh_token = new_tokens.credentials.refresh_token;
          auth_info.access_token = new_tokens.credentials.access_token;
          this.ev.emit('update-credentials', { credentials: new_tokens.credentials, status: new_tokens.status });
        }
  ...
  async search(query, options = { client: 'YOUTUBE', period: 'any', order: 'relevance', duration: 'any' }) {
    ...
    const refined_data = new Parser(response.data, {
  ...
  */
  async getDetails(id) {
    ...
    const refined_data = new Parser(data, { client: 'YOUTUBE', data_type: 'VIDEO_INFO', desktop_v: false }).parse();
  ...
  download(id, options = {}) {
    ...
    Actions.getVideoInfo(this, { id, is_desktop: true }).then(async (video_data) => {
      ...
      if (!selected_format) {
        ...
        const refined_data = new Parser(video_data, { client: 'YOUTUBE', data_type: 'VIDEO_INFO', desktop_v: true }).parse();
