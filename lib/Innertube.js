...
const HomeFeed = require('./core/HomeFeed');
const { Trending } = require('./core/Trending');
...
class Innertube {
  ...
  async search(query, options = { client: 'YOUTUBE', period: 'any', order: 'relevance', duration: 'any' }) {
    const response = await this.actions.search(options.client, { query, options });
   ...
   * @returns 
   */
  async _getDetails(video_id) {
    if (!video_id) throw new Utils.MissingParamError('Video id is missing');

    const response = await this.actions.getVideoInfo(video_id);

    return new Video(this, response);
  }
  ...
  async getChannel(id) {
    ...
    return new Channel(this, response.data);
  }
  ...
  async getHomeFeed() {
    ...
    return new HomeFeed(this, response.data);
  }
  ...
  async getTrending() {
    ...
    return new Trending(this, response.data);
  }
  ...
  async getPlaylist(playlist_id, options = { client: 'YOUTUBE' }) {
    ...
    return new Playlist(this, response.data, options.client);
  }
  ...
  download(id, options = {}) {
    ...
    this.actions.getVideoInfo(id).then(async (video_data) => {
