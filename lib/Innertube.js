...
class Innertube extends EventEmitter {
  ...
  signIn(credentials = {}) {
    return new Promise(async (resolve, reject) => {
      ...
      if (credentials.access_token && credentials.refresh_token) {
        let token_validity = await oauth.checkTokenValidity(credentials.expires);
        if (token_validity === 'VALID') {
          ...
          oauth.on('refresh-token', (data) => {
            ...
            const expiration_date = new Date(new Date().getTime() + data.expires * 1000);
  ...
  async search(query, options = { period: 'any', order: 'relevance', duration: 'any' }) {
    ...
    const response = await Axios.post(`${Constants.URLS.YT_BASE_URL}/youtubei/v1/search${this.logged_in && this.cookie.length < 1 ? '' : `?key=${this.key}`}`, JSON.stringify({ context: this.context, params: Utils.encodeFilter(options.period, options.duration, options.order), query }), Constants.INNERTUBE_REQOPTS({ session: this })).catch((error) => error);
