...
export interface PlaylistCollaboratorParams {
  params: PlaylistCollaboratorParams_Params | undefined;
}

export interface PlaylistCollaboratorParams_Params {
  playlistId: string;
}
...
function createBasePlaylistCollaboratorParams(): PlaylistCollaboratorParams {
  return { params: undefined };
}

export const PlaylistCollaboratorParams: MessageFns<PlaylistCollaboratorParams> = {
  encode(message: PlaylistCollaboratorParams, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.params !== undefined) {
      PlaylistCollaboratorParams_Params.encode(message.params, writer.uint32(1106).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): PlaylistCollaboratorParams {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBasePlaylistCollaboratorParams();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 138: {
          ...
          message.params = PlaylistCollaboratorParams_Params.decode(reader, reader.uint32());
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
...
function createBasePlaylistCollaboratorParams_Params(): PlaylistCollaboratorParams_Params {
  return { playlistId: "" };
}

export const PlaylistCollaboratorParams_Params: MessageFns<PlaylistCollaboratorParams_Params> = {
  encode(message: PlaylistCollaboratorParams_Params, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): PlaylistCollaboratorParams_Params {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBasePlaylistCollaboratorParams_Params();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
