...
export default class VideoInfo extends MediaInfo {
  ...
  constructor(data: [ApiResponse, ApiResponse?], actions: Actions, cpn: string) {
    ...
    if (!this.basic_info.channel && owner) {
      // `Author#url` is derived from the navigation endpoint's web metadata, which the TV
      // client often omits, so fall back to the canonical channel URL built from the id.
      this.basic_info.channel = {
        id: owner.id,
        name: owner.name,
        url: owner.url || `${Constants.URLS.YT_BASE}/channel/${owner.id}`
      };
    }
